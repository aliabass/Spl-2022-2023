...
class TableTest {
    ...
    private void placeSomeTokens() {
        table.PlaceTokensForTesting(1, 0);
        table.PlaceTokensForTesting(1, 1);
        table.PlaceTokensForTesting(1, 2);
    }

    private void removeAllTokens() {
        for (int i = 0; i < table.slotToCard.length; i++) {
            if (table.ifTokenExists(1, i)) {
                table.removeToken(1, i);
            }
        }
    }

    private int numOfTokensOnTable() {
        int ans = 0;
        for (int i = 0; i < table.slotToCard.length; i++) {
            if (table.ifTokenExists(1, i)) {
                ans++;
    ...
    //added tests
    @Test
    void placeTokens_SomeAreFilled() {
        try {
            placeCard_AllSlotsAreFilled();
        }catch (InterruptedException ignored){}
        placeSomeTokens();
        int num = numOfTokensOnTable();
        assertEquals(3, num);//expected,actual
    }

    @Test
    void RemoveTokens_NoTokensOnTable() {
        placeSomeTokens();
        removeAllTokens();
        int num = numOfTokensOnTable();
        assertEquals(0, num);
